...
title_tag = $j(document).find("title").clone().wrap('<p>').parent().html();
if (title_tag === undefined) {
  title_tag = "****** title tag not found *****";
}
// Compute Anchor Data
$j('a').not(genClsCarolina(carolina508_uuid) + ' a').map(function(index, value){
    var anchor = {};
    anchor["title"]   =  $j(this).attr("title");
    if (anchor["title"] === undefined) {
      anchor["title"] = "****** title attribute not found ******";
    }
    anchor["code"]    =  $j(this).clone().wrap('<p>').parent().html();
...
// Compute Image Data
$j('img').not(genClsCarolina(carolina508_uuid) + ' img').each(function(index, value){
   var image = {};
   var current_width   = $j(this).css("width");
   var current_height  = $j(this).css("height");
   var not_found = "";
   ...
   image["current_hw"]  = ' ( h: ' + current_height + ' &amp; w: ' + current_width + ' ) ';
    ...
    image["code"]    =  $j(this).clone().wrap('<p>').parent().html();
    image["alt_text"]   =  $j(this).attr("alt");
    if (image["alt_text"] === undefined) {
      image["alt_text"] = "****** alt attribute not found *****";
      image["alt_text_length"] = "***";
    } else {
    image["alt_text_length"]   = $j(this).attr("alt").length;
    }
...
// Compute Form Data
$j('form').not(genClsCarolina(carolina508_uuid) + ' form').each(function(index, value){
    var all_buttons = [];
    var all_inputs = [];
    var form_data = {};
    form_data["action_info"] = $j(this).attr('action');
    $j(this).find("button").each(function(index, value){
      var button_data = {};
      button_data["id"] = $j(this).attr('id');
      button_data["text"] = $j(this).text();
      all_buttons.push(button_data);
    });
    $j(this).find("input").each(function(index, value){
      var input_data = {};
      input_data["id"] = $j(this).attr('id');
      input_data["name"] = $j(this).attr('name');
      input_data["type"] = $j(this).attr('type');
      input_data["is_focused"] = $j(this).is(":focus");
...
// Create Report
$j("body").append('<div class="'+carolina508_uuid+'" id="'+carolina508_project_timestamp+'"></div>');
$j(carolina508_id).append('<div class="'+ carolina508_report +'">CAROLINA508 <i>'+ carolina508_project_name +'</i></div>');
...
// Title HTML
$j(carolina508_id).append('<div class="'+ carolina_section +'"><div id="'+ c_s_title +'"></div></div>');
$j(genIdCarolina(c_s_title)).append('<table class="'+ fixed_layout +'"><tr><td class="'+ table_title +'" id="'+ c_s_title_heading_info +'"></td><td class="'+table_references+'" id="'+ c_s_title_references_info +'"></td></tr><tr><td colspan="2" class="'+table_instructions+'" id="'+ c_s_title_instructions_info +'"></td></tr><tr><td class="'+table_result+'" id="'+ c_s_title_result_title +'"></td><td class="'+table_result+'" id="'+ c_s_title_result_info +'"></td></tr></table>');
// Title Bind Data
$j(genIdCarolina(c_s_title_heading_info)).html("title tag");
$j(genIdCarolina(c_s_title_references_info)).html("<b>References:</b>");
$j(genIdCarolina(c_s_title_instructions_info)).html("<ul><li>If each page on the website has the same title, screen readers would not be able to easily distinguish different pages.</li></ul>");
$j(genIdCarolina(c_s_title_result_title)).html("PAGE RESULT");
$j(genIdCarolina(c_s_title_result_info)).html(genCodeCarolina(title_tag));
...
// Anchor HTML
$j(carolina508_id).append('<div class="'+ carolina_section +'"><div id="'+ c_s_anchor +'"></div></div>');
$j(genIdCarolina(c_s_anchor)).append('<table class="'+ fixed_layout +'"><tr><td class="'+ table_title +'" id="'+ c_s_anchor_heading_info +'"></td><td class="'+table_references+'" id="'+ c_s_anchor_references_info +'"></td></tr><tr><td colspan="2" class="'+table_instructions+'" id="'+ c_s_anchor_instructions_info +'"></td></tr><tr><td colspan="2" id="'+ c_s_anchor_table +'"></td></tr></table>');
// Anchor Bind Data
$j(genIdCarolina(c_s_anchor_heading_info)).html("anchor tag");
$j(genIdCarolina(c_s_anchor_references_info)).html("<b>References:</b>");
$j(genIdCarolina(c_s_anchor_instructions_info)).html("<ul><li>Links should always have description in the title attribute.</li><li>Avoid making users open links in new window as there is no chance of back button.</li><li>Accessibility person will face difficulty because of this. If it is unavoidable include that in title attribute like this: 'Your Link Description'<br>followed by (Opens in new window)</li></ul>");
...
$j(carolina508_id).append('<div class="'+ carolina_section +'"><div id="'+ c_s_image +'"></div></div>');
$j(genIdCarolina(c_s_image)).append('<table class="'+ fixed_layout +'"><tr><td class="'+ table_title +'" id="'+ c_s_image_heading_info +'"></td><td class="'+table_references+'" id="'+ c_s_image_references_info +'"></td></tr><tr><td colspan="2" class="'+table_instructions+'" id="'+ c_s_image_instructions_info +'"></td></tr><tr><td colspan="2" id="'+ c_s_image_table +'"></td></tr></table>');

// Image Bind Data
$j(genIdCarolina(c_s_image_heading_info)).html("image tag");
$j(genIdCarolina(c_s_image_references_info)).html("<b>References:</b>");
$j(genIdCarolina(c_s_image_instructions_info)).html("<ul><li>Images with alt=' ' are ignored by screen readers as they are considered decorative.</li><li>Limit alt(alternative) text eplanation to 125 characters. Jaws treats it different if it exceeds.</li></ul>");
...
$j(genClsCarolina(t_r_m_img_alt_text_description)).css({'width':'100%'});
$j(genClsCarolina(t_r_m_img_info)).css({'width':'200px'});
$j(genClsCarolina(t_r_m_img_alt_text_length)).css({'width':'120px'});


// Report CSS
$j(carolina508_id).css({'background-color':carolina508_bg_hex,'margin-top':'0px','min-height':'200px','border-top':'5px solid #001F3F','border-bottom':'2px dashed #001F3F','font-size':'14px','color':'#000','font-family': '"Lucida Sans Unicode", "Lucida Grande", sans-serif','padding-bottom':'200px'});
$j(genClsCarolina(carolina508_report)).css({'font-size':'28px','color':'#001F3F','border-bottom':'1px solid #001F3F','padding':'20px', 'letter-spacing': '2px', 'text-align': 'center', 'background-color': '#FFFFFF'});
$j(genClsCarolina(carolina_section)).css({'display':'block','margin-left':'auto','margin-right':'auto','padding':'40px 40px 0px 40px','width':'1140px'});
$j(carolina508_id + ' table').css({'border-collapse':'collapse'});
$j(carolina508_id + ' td img').css({'width':'100%','height':'auto'});
$j(carolina508_id + ' table, th, td').css({'border-style':'solid','border-width':'1px','border-color':carolina508_border_hex,'word-break':'break-all'});
$j(carolina508_id + ' ul li').css({'margin-top':'10px'});
$j(genClsCarolina(fixed_layout)).css({'table-layout':'fixed','width':'100%','border-style':'solid'});
$j(genClsCarolina(table_title)).css({'background-color':carolina508_clr_hex,'text-align':'center','color':'#FFF','width':'200px','letter-spacing':'2px','font-size':'24px'});
$j(genClsCarolina(table_references)).css({'background-color':'#FFF','color':'#000','font-size':'16px','padding':'10px'});
$j(genClsCarolina(table_instructions)).css({'background-color':'#FFF','color':'#000','font-size':'14px','padding':'10px 25px 10px 10px'});
$j(genClsCarolina(table_result)).css({'background-color':'#FFF','font-size':'16px','padding':'10px','text-align':'center','letter-spacing':'2px'});
$j(carolina508_id + ' textarea').css({'background-color':'#FFF','border':'none','width':'100%','padding':'20px','height':'70px', 'font-family': 'monospace','max-width':'100%','max-height':'200px','font-size':'14px','color':'#555'});
$j(genClsCarolina(table_results_matrix)).css({'table-layout':'fixed','width':'100%'});
//These two need some work!
$j(genClsCarolina(table_results_matrix) + ' table').css({'border-collapse':'collapse','border-style':'hidden'});
$j(genClsCarolina(table_results_matrix) + ' th').css({'background-color':'#FFF','text-align':'center','border-top':'none','padding':'10px 0px'});
$j(genClsCarolina(table_results_matrix) + ' td').css({'background-color':'#FFF','border-top':'none','padding':'10px','text-align':'center'});
$j(genClsCarolina(table_results_matrix_no_width)).css({'width':'60px','word-break':'break-all'});
$j(genClsCarolina(table_results_matrix_details_width)).css({'width':'100px'});
